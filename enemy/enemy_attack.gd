...
func enter(): # 當狀態機切換進入攻擊狀態時執行
	...
	# 🌟 [修復 1] 不再停用 Hitbox！讓 Hitbox 保持常開，玩家蓄力期撞上來也會被彈開
...
func state_physics_update(_delta: float): # 物理幀更新邏輯 (每秒執行 60 次)
	if is_charging: # 階段一：原地蓄力中
		character.velocity = Vector2.ZERO # 🌟 [修復 2] 每幀鎖死速度，防止玩家硬推
...
func _start_dash(): # 處理從蓄力切換到衝刺的瞬間
	...
	character.has_hit_player = false # 🌟 [修復 3] 正式開始衝刺時重置標記，確保衝刺能完整執行
